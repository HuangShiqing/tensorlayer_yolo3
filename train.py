            ...
            loss_c = tf.reduce_sum(
                object_mask * tl.cost.binary_cross_entropy(adjusted_out_c, adjusted_true_c) + (
                        1 - object_mask) * tl.cost.binary_cross_entropy(adjusted_out_c,
                                                                        adjusted_true_c) * ignore_mask) / batch_size
...
def main():
    ...
    y_true_pb = [tf.placeholder(tf.float32, [None, 52, 52, 3, 5 + C]),
                 tf.placeholder(tf.float32, [None, 26, 26, 3, 5 + C]),
                 tf.placeholder(tf.float32, [None, 13, 13, 3, 5 + C])]
    net_out = infenence(input_pb)
    loss_op = yolo3_loss(net_out, y_true_pb)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epoch):
            ...
            for img, lable_box in data_yield:
                ...
                loss, _ = sess.run([loss_op, train_op], feed_dict={input_pb: img, y_true_pb: lable_box})
                step += 1
                ...
                # 每step打印一次该step的loss
                print("Loss %d  : Epoch %d : Step %d  took %fs" % (loss, epoch, step, time.time() - start_time))
